...
public class SumOfKMirror {
    public static void main(String[] args) {
        System.out.println(kMirror(5,2));
    }
     static long createPalindrome(long num, boolean odd) {
        long x = num;
        if (odd) x /= 10;
        while (x > 0) {
            num = num * 10 + x % 10;
    ...
    static boolean isPalindrome(long num, int base) {
    ...
    static long kMirror(int k, int n) {
        long sum = 0;
